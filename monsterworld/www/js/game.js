...
(function() {
    var m = [],
        ...
        u = Math.PI,
        F, M = "#",
        N = ['blue', 'blue1', 'gray' , 'pink', 'lightblue', 'green'];

        var score_container = document.getElementsByClassName("scoreboard")[0],
            scorecard = document.getElementsByClassName("scorecard")[0];
...
//        a.putImageData(f, b, c)
    ...
    window.onload = function() {
        ...
        b.addColorStop(0, "#EFE3B8");
        b.addColorStop(1, "#FCEBAC");
        var c = a.createLinearGradient(0, 0, 0, 450);
        c.addColorStop(0, "#ECECEC");
        c.addColorStop(1, "#ECECEC");
        for (var f = 0; f < 7; f++) {
            t[f] = [];
            m[f] = [];
            for (var e = 0; e < 7; e++) {
        ...
        if (!(!i && !o && !F))
            if (F)
                ...
                for (a = b = 0; a < o[j]; a++) {
                    c = t[o[a].x][o[a].y];
                    if (c.removing)
                        c.style.opacity = (c.removing -= 5) / 100;
                    else {
                        c.removing = 100;
                        m[o[a].x][o[a].y] = 0;
                        n.push({
    ...
    function ba(a, b, c, f) {
        ...
        for (l = 0; 1;) {
            ...
            if (d.x == c && d.y - 1 == f || d.x == c && d.y + 1 == f || d.x - 1 == c && d.y == f || d.x + 1 == c && d.y == f)
                break;
            if (l > 81)
                break
        }
...
//        alert("Game over\n Your score is: " + y);
    ...
    function Y(a) {
        ...
        for (var b = p.cookie.split(";"), c = 0, f = b[j]; c < f; c++) {
            for (var e = b[c]; e.charAt() == " ";)
                e = e.substring(1, e[j]);
            if (!e.indexOf(a))
                return e.substring(a[j], e[j])
        }
        return null
    }
